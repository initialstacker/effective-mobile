...
RUN apt-get update && apt-get install -y --no-install-recommends \
    git unzip curl supervisor libsqlite3-dev libzip-dev libpng-dev libfreetype6-dev libjpeg-dev zlib1g-dev libonig-dev redis-tools ca-certificates libicu-dev \
    && docker-php-ext-configure intl \
    && docker-php-ext-install -j$(nproc) intl gd pdo pdo_sqlite mbstring zip exif opcache pcntl sockets \
